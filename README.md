...
桌面版还可以转换音频，或把多段视频按顺序合并成一条。图片、PDF 和 Word 也能在本机转成常用格式。

视频和音频能转哪些格式，取决于捆绑的 FFmpeg 能否解码。图片、PDF 和 Word 走独立文档引擎，不经过 FFmpeg。不是云服务，也不保证「任意编码都能完美转出」。
...
默认输出目录是「下载 / LiteTrans」。下次打开会记住上次的格式、画质和输出位置。

桌面版左侧栏有「历史」。关闭窗口后，历史记录仍然保留。界面语言在「我的」里切换。可在「我的」打开局域网，让同一网络的浏览器查看并下载已完成文件；默认关。
